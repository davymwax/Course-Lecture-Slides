...
\item Paraphrasing sentences using Quantified Relational Logic
\end{itemize}
...
>- This was useful. But it was also too simple. It was too simple because we abstracted away the internal composition of sentences except the truth-functional connectives (\textcolor{red}{and, either...or..., if..., then...})
...
>- Fundamental to quantified predicate logic is the notion of \textcolor{red}{a variable} and \textcolor{red}{a quantifier}, which we use \textcolor{red}{to generalize}.
...
>- We will not have to represent anything crazy like (12) and (13) for this class, but you will need to have an idea similar to what happens in high school algebra to understand what is going on.
...
>- We introduce the relational symbol \textcolor{red}{$W$} for the relation "is wise" and paraphrase write (1) and (2) as:\begin{itemize} \item[(14)] $Wx$ \,\,\, Read as "$x$ is $W$" \item[(15)] $Wy$ \,\,\,  Read as "$y$ is $W$" \end{itemize} 
...
>- Introduce symbols the relational symbol \textcolor{red}{$F$} for the relation "is female" and paraphrase (3) and (4):
...
\item[(17)] $\forall x Wx$ \,\,\, Read as "Every $x$ is a $W$".
\end{itemize}

# Quantified Relational Logic: Predicates and Relations

Using appropriate symbols for quantifiers, variables and relations, paraphrase or formalize the following sentences in quantified predicate logic:
\begin{itemize}
\item[(3)] Hypatia is female.
\item[(4)] Cavendish is female.
\end{itemize}
...
\item[(19)] Ottoline met Russell.
\end{itemize} we introduce symbols for 2-place relations and use different variables for all the distinct subjects or objects in the relation.
...
Using the symbol $M$ for the relation $<x$ met $y>$ paraphrase (19).

# Quantified Relational Logic: The Logic Itself

>- We are now in a position do describe quantified relational logic for \textcolor{red}{one} or \textcolor{red}{two} place relations. Remember that to specify a logic I need to tell you (1) the formal symbols, (2) the transformation rules, and (3) closure condition.

>- The \textcolor{red}{formal symbols} are:
\begin{itemize}
\item \textcolor{red}{x, y, z} as symbols for \textcolor{red}{variables}, which range over individuals or objects.
\item Upper case letters \textcolor{red}{$(P, Q, R, S, T, \dots, A, B, C, D,\dots)$} of the English alphabet as symbols for \textcolor{red}{relations} (one or two place only).
\item Truth-functional connectives: \textcolor{red}{$\vee$, $\wedge$, $\lnot$, $\to$}
...
\begin{enumerate}
\item For any predicate symbol $P$ and for any two place relation symbol $R$ and variables $x$ or $y$, $Px$ and $Rxy$ is a formula. In these formulas, $x$ and $y$ are said to \textcolor{red}{"free"} variables because there are no quantifiers to which they are \textcolor{red}{bound}.
\item If $Px$ is a formula, then $\exists x Px$ is a formula and $\forall x Px$ is a formula. Rule 2 is known as \textcolor{red}{binding} any free variable in $Px$ by a quantifier.
\item If $Rxy$ is a formula, then $\exists x Rxy$ is a formula and $\forall x Rxy$ is a formula. Here $y$ is a free variable but $x$ is bound.
\item If $\mathcal{F}$ is formula by rule 3, then the result of \textcolor{red}{binding} any free variable in $\mathcal{F}$ is a formula.
\item A formula $\mathcal{F}$ formed by either rule 1, 2, 3 or 4 is called an \textcolor{red}{atomic formula}. If $\mathcal{F}$ and $\mathcal{G}$ are atomic formulas, then $(\mathcal{F} \vee \mathcal{G})$, $(\mathcal{F} \wedge \mathcal{G})$, $\lnot \mathcal{F}$, $(\mathcal{F} \to \mathcal{G})$ are \textcolor{red}{molecular formulas}.
\item If $\mathcal{H}$ is a molecular formula, then the result of \textcolor{red}{binding} any free variable in $\mathcal{H}$ is a molecular formula. 
\end{enumerate}

# Quantified Relational Logic: The Logic Itself

>- The \textcolor{red}{Closure Condition} says that a formula of quantified relational logic is either an atomic formula or a molecular formula built from atomic formulas by finite applications of rule 5 and 6. Nothing else is a molecular formula.
