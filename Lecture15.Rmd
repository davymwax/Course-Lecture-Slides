...
\item Strict Dominance Principle
...
# Decision Problems Under Ignorance
...
>- The relevant sense of "information" here is information about the \textcolor{red}{probability distribution} of states. 

>- The relevant sense of "uncertainty" here is that we don't \textcolor{red}{know} \textcolor{red}{the probabilities} with which the states will occur \textcolor{red}{with certainty}.
...
>- Weak Dominance: $A_{i} \succeq A_{j}$ if and only if $u(A_{i} | S) \geq u(A_{j} | S)$ for \textcolor{red}{every} state $S$ (at least as good)

>- Strict Dominance: $A_{i} \succ A_{j}$ if and only if (1) $u(A_{i} | S_{n}) \geq u(A_{j} | S_{n})$ for \textcolor{red}{every} state $S_{n}$ (at least as good) and (2) there exists a state $S_{m}$ such that $u(A_{i} | S_{m}) > u(A_{j} | S_{m})$ (at least one better).
...
\item $A_{1} \succeq A_{3}$?
\item $A_{1} \succ A_{2}$?
\item $A_{3} \succeq A_{2}$?
\end{itemize}
