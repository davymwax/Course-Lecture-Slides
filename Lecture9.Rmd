...
>- Last time we introduced the formal language of set theory. Set theory is the study of collections. We said that we need to be careful in specifying what sorts of collections count as sets. 

>- The best way to study what sorts of collections count as sets is to proceed "axiomatically". This means that we list down the rules that "an acceptable set" should obey. These rules are called "axioms". Later you will see axioms coming up again in saying what a probability function is.
...
>- With that in mind, let me add that none of the sets we're going to study in this class will be ruled about by the axiomatic approach. So we're good!
...
>- To write what the members or elements of a set are, we can just list them \textcolor{red}{without repetition}. For example, the possible outcomes of the roll of a die can be written down as:
...
\[
\{ x \, | \, Px\}
\] which is read as "The set of those $x$'s that are $P$'s" or "The set $x$'s such that $Px$".


# Operations with sets: Writing down sets

>- Here are examples: \begin{itemize} \item \[\mathbb{N} = \{n \, | \, n \,\text{is a natural number} \} = \{0, 1, 2, 3, 4, 5, 6, \dots \} \] 
...
>- Recall that the union of two sets $A$ and $B$, $(A \cup B)$, is the set whose members belong to either $A$ or $B$.

>- Suppose we're dealing with the set of possible outcomes of the roll of one die. This is the set $\{1, 2, 3, 4, 5, 6 \}$

>- Let $A = \{2, 3\}$ and $B = \{4, 5\}$. Then $(A \cup B) = \{ 2, 3, 4, 5\}$. To verify that this is the union of $A$ and $B$, we need to check the members of $(A \cup B)$ are in either $A$ or $B$.
...
>- Recall that the intersection of two sets $A$ and $B$, $(A \cap B)$, is the set whose members belong to both $A$ and $B$.

>- Suppose we're dealing with the set of possible outcomes of the roll of one die. This is the set $\{1, 2, 3, 4, 5, 6 \}$

>- Let $A = \{1, 2, 3, 4\}$ and $B = \{2, 3\}$. Then $(A \cap B) = \{ 2, 3\}$. To verify that this is the intersection of $A$ and $B$, we need to check that \textcolor{red}{all} the members of $(A \cap B)$ are in both $A$ and $B$.
...
>- Exercise. Write down the complement of $A$ for $\mathcal{U}$ given in the problem: \begin{itemize} \item $A = \{1, 2\}$, $\mathcal{U} = \{1, 2, 3, 4, 5, 6 \}$ 
