...
>- Exercise. Let $E$ be an incident of myocardial infarction (i.e., heart attack) and $E^{c}$ an no incident of myocardial infarction. Find $RR(E)$ for physicians who took the placebo compared to those who took aspirin. Interpet the result.
...
>- Here is the table. \begin{center}
...
>- In $2 \times 2$ tables the \textcolor{red}{odds ratio}, $\Omega$ is the appropriate measure of association for detecting possible risk factors in \textcolor{red}{a case-control study}.\[
\Omega = \frac{\text{Odds of outcome E for those in Group 1}}{\text{Odds of outcome E for those in Group 2}} = \frac{\frac{P_{1}(E)}{1 - P_{1}(E)}}{\frac{P_{2}(E)}{1 - P_{2}(E)}}
\]

>- The odds ratio $\Omega$ can be calculated directly from the cell counts using the formula:\[
\Omega = \frac{\frac{P_{1}(E)}{1 - P_{1}(E)}}{\frac{P_{2}(E)}{1 - P_{2}(E)}} = \frac{n_{11}/n_{12}}{n_{21}/n_{22}} = \frac{n_{11}n_{22}}{n_{12}n_{21}}
\]

>- Because $\Omega = \frac{n_{11}n_{22}}{n_{12}n_{21}}$ some authors use the term odds ratio interchangeably with the \textcolor{red}{cross-product ratio}.

# Intepreting the Odds Ratio

>- The odds ratio can equal any non-negative number. When X and Y are independent $\Omega = 1$. 

>- Odds ratios on each side of 1 reflect certain types of associations. When $\Omega > 1$, the odds of outcome $E$ are higher for Group 1 than for Group 2. 

>- For instance, when $\Omega = 4$, the odds of outcome $E$ in Group 1 are four times the odds of outcome $E$ in Group 2. 

>- Values of $\Omega$ farther from 1 in a given direction represent a stronger association. An odds ratio of 4 is farther from independence than an odds ratio of 2, and an odds ratio of 0.25 is farther from independence than an odds ratio of 0.50.
...
>- Without controlling for $Z$, an observed XY association may merely reflect an association of $Z$ with X and Y. This is \textcolor{red}{especially vital for observational studies}, for which one cannot remove effects of such variables by randomly assigning subjects to different treatments.
...
>- Exercise.\begin{itemize} \item Write down the marginal table for the association between the defendant's race and whether they received the death penalty verdict. Calculate the odds ratio. This is called the \textcolor{red}{marginal odds ratio}. Interpret it. \item Write down the partial table if the victim's race is white and calculate the odds ratio. This is also a \textcolor{red}{marginal odds ratio}. Interpret it. 
\end{itemize}

# Simpson's Paradox: Second Illustration Cont'd.

>- Exercise (Cont'd.) \begin{itemize}
\item Write down the partial table if the victim's race as black and calculate the odds ratio. This is called the \textcolor{red}{conditional odds ratio} Interpret it. \item Compare the direction of conditional association to the direction of marginal association, is this Simpson's Paradox? \end{itemize}
